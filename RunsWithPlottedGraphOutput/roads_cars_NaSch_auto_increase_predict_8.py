...
for af in AUTO_FRACS:
    ...
    print()
    print(f"Predictability metrics (flow = entered):")
    pred = summ["predictability"]
    print(f"   CV={pred['CV']:.4f}  lag1_autocorr={pred['lag1_autocorr']:.4f}")
    print(f"   naive_MSE={pred['naive_MSE']:.4f}  normalized_MSE={pred['normalized_MSE']:.6f}")
    print()
    print()


# CV - Relative variability of flow decreased as autonomous percentage increased.
...
# Lag-1 autocorr may remain negative — that’s structural oscillation.

# As auto_frac increases:

# Throughput ↑

# Jam clusters ↓ strongly

# Relative volatility ↓ (after mid-range)

# Normalized forecast error ↓ (after mid-range)

# Oscillation structure becomes stronger (more negative lag-1)